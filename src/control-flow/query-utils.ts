...
import { BlockMatcher } from "./block-matcher.js";
// I may remove this one entirely - this is just for testing
export function extractCapturedTextsByTag(
  func: SyntaxNode,
  query: string,
  tag: string,
): string[] {
  const matcher = new BlockMatcher((_syntax: SyntaxNode) => ({
    entry: "",
    exit: null,
  }));

  const allMatches = matcher.tryMatchAll(func, query, { maxStartDepth: 1 });

  const allCapturedTexts: string[] = [];
  for (const match of allMatches) {
    const capturedNodes = match.getSyntaxMany(tag);
    const texts = capturedNodes
      .filter((node: SyntaxNode) => node.text)
      .map((node: SyntaxNode) => node.text);
    allCapturedTexts.push(...texts);
  }

  return allCapturedTexts;
}
